...
function replaceStrongWithH4(content: string): string {
    const parser = new DOMParser();
    const doc = parser.parseFromString(content, "text/html");
    const strongElements = doc.getElementsByTagName("strong");
    for (let i = 0; i < strongElements.length; i++) {
      const strongElement = strongElements[i];
      const h4Element = doc.createElement("h4");
      h4Element.textContent = strongElement.textContent;
      strongElement.replaceWith(h4Element);
    }
...
const ProductDescription = (props:any) =>{
   ...
   const replaceContent = replaceStrongWithH4(content);
  
    return(
      <Background>
        {/* html 마크업 언어 파싱해주는 속성 */}
         <div dangerouslySetInnerHTML={{ __html: replaceContent }} />
      </Background>
    )
  }
